...
// POSSIBLE SOLUTION TO ABOVE: --> x.reverse.foldLeft(y)((t, h) => h :: t)
  ...
  def foldRight [A,B] (xs:List[A], e:B, f:(A,B)=>B) : B = {
    ...
    xs match {
      case Nil => e
      case y::ys => f(y, ys.foldRight(e)(f))
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    ...
    xs match {
      case Nil => ""
      case hd::tl => hd + foldRight(tl, delimiter, f)
    }
  }
  ...
  def firstNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    xs match {
      ...
      case hd::tl => if (hd >= a) hd else firstNumGreaterThan(a, tl)
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    def firstIndexHelper(a : Int, xs : List[Int], i : Int): Int = {
      xs match {
        case Nil => throw new RuntimeException
        case hd::tl => if (hd >= a) i else firstIndexHelper(a, tl, i+1)
      }
    }
