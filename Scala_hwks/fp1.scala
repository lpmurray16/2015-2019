...
object fp1 {
  ...
  val factTest = fact(1)::fact(2)::fact(3)::fact(4)::fact(5)::Nil
 ...
 def fib (n: Int): Int = {
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = {
    ...
    xs match {
      case Nil => accumulator
      case x::xs => maxTailRecursiveAux(if(x > accumulator) x else accumulator, xs)
    }
  ...
  def otpu (start : Int, end : Int) : List[Int] = {
    // TODO: Provide definition here.
    if (start < end) List() else List.range(start, end)
  }
}
