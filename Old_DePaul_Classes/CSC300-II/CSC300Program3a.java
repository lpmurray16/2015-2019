...
public class CSC300Program3a {
	...
	public static int numUnique (int[] list) {
		...
		ArrayList<Integer> diffNums = new ArrayList<>(); //creates a list for unique int values.
		
		for(int i = 0; i < list.length; i++){
	        if(!diffNums.contains(list[i])){  // checks is value is in created list.
	            diffNums.add(list[i]);        // if value is NOT in created list, it adds it.
	        }
	    }
		
		if (diffNums.size() == 0) return 0; // nothing in array. return 0.
		
		// else
		
		return diffNums.size(); // if any value is added to array, return size.
	...
	public static int[] deDupAndReverse (int[] list) {
		...
		ArrayList<Integer> uniqueVals = new ArrayList<>(); //creates ArrayList to help.
		for(int i = list.length - 1; i >= 0; i--){ //increments backwards for reverse order.
	        if(!uniqueVals.contains(list[i])){  // checks if value is in created ArrayList.
	            uniqueVals.add(list[i]);        // if value is NOT in created list, it adds it.
	        }
	    }
		
		int [] revArray = new int[numUnique(list)]; //creates a new primitive int array, size taken from numUnique function.
		
		for(int i = 0; i < uniqueVals.size(); i++) {  // uses size of ArrayList, just for clarity of mind.
		    if (uniqueVals.get(i) != null) {          // if pointer finds null it will exit.
		        revArray[i] = uniqueVals.get(i);      //place value from ArrayList into the new array
		    }
		}
		
		return revArray;  // returns the final array in reverse order.
	}
