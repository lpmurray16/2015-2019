...
public class SequentialSearchST<Key extends Comparable<Key>, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        if (key == null) throw new IllegalArgumentException("first argument to put() is null"); // cannot enter null key
        if (val == null) { // if the val input is null, finds key associated and deletes it
            delete(key);
            return;
        
        }
        if(n==0){ // if the List is empty
        	first = new Node(key, val, null); // put the new key and val inside it
        	n++; // grows in size by 1
        }
        else {
        
        	for (Node x = first; x != null; x = x.next) { // iterates through all available Nodes
            	if (key.equals(x.key)) {  // does the equals check on keys
                	x.val = val; // replaces values
                	return; // exits loop and method
            	}
            	else if(key.compareTo(x.key) < 0) { // checks to see if new key is less than first.key
            		first = new Node(key, val, first);
            		n++;
            		return;
            	}
            	else if(key.compareTo(x.key) > 0 && x.next != null && key.compareTo(x.next.key) < 0) { // finds where to fit if in middle
            		Node insert = new Node(key, val, x.next);
            		x.next = insert;
            		n++;
            		return;
            	}
            	else if(x.next==null) { // puts the new Node at the end and points to null
    ...
    public void delete(Key key) {
        ...
        for(Node x = first.next; x != null; x = x.next) { // iterate through the none empty List
