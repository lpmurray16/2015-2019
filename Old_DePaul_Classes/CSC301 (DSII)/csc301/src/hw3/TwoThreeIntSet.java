...
public class TwoThreeIntSet {
	...
	private class Node {
		 ...
		 * @param left
		 *            the root of the subtree less than the item
		 * @param right
		 *            the root of the subtree greater than the item
		 */
	 ...
	 * 
	 * @param n
	 *            the root of a 2-3 tree into which we will insert a number
	 * @param item
		...
		// If not a leaf node, we will need to insert in one of our subtrees.
			...
			if (n.nodeType == 2) {
				if (item < n.items[0]) {
					...
					else { // this else statement is ONLY if the node is a 4 node
				...
				} else { // this else statement is ONLY if the item inserted was not < items[0]
				...
				if(item < n.items[0]) {
					...
					if(result.nodeType != 4) {
						...
						//split
						Node newLeft = new Node(result.items[0], result.subtrees[0], result.subtrees[1]);
						Node newLeftMid = new Node(result.items[2], result.subtrees[2], result.subtrees[3]);
						n.items[2] = n.items[1];
						n.items[1] = n.items[0];
						n.items[0] = result.items[1];
						n.subtrees[3] = n.subtrees[2];
						n.subtrees[2] = n.subtrees[1];
						n.subtrees[1] = newLeftMid;
						n.subtrees[0] = newLeft;
						n.nodeType = 4;
					}
				}
					...
					if(result.nodeType!=4) {
						...
						//if it IS a 4 node
						//split
						Node newLeftMid = new Node(result.items[0],result.subtrees[0],result.subtrees[1]);
						Node newRightMid = new Node(result.items[2],result.subtrees[2],result.subtrees[3]);
						n.items[2]=n.items[1];
						n.items[1]=result.items[1];
						n.subtrees[3] = n.subtrees[2];
						n.subtrees[2] = newRightMid;
						n.subtrees[1] = newLeftMid;
						n.nodeType = 4;
					}
				}
					...
					if(result.nodeType!=4) {
						...
						//if it IS a 4 node
						//split
						Node newRightMid = new Node(result.items[0],result.subtrees[0],result.subtrees[1]);
						Node newRight = new Node(result.items[2],result.subtrees[2],result.subtrees[3]);
				...
				return n;//throw new RuntimeException("Implement me!");
			} else
				throw new RuntimeException("ERROR: " + n.nodeType + "-node found while inserting");
