...
public class LinearProbingHashST<Key, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        for (i = hash(key); keys[i] != null; i = (i + 1) % m) {
            if(key.equals(keys[i])) { // If key==key, then replace value
            	vals[i] = val;
            	return;
            } 
            else if (keys[i] != null && vals[i] == null) { // reuse the lazy key
            	keys[i] = key;
            	vals[i] = val;
            	n++;
            	return;
        ...
        // below executes if above loop doesn't find: key==key or key==lazyDel
        // The counter "i" will be on a key==null spot
        keys[i] = key;
        vals[i] = val;
        n++;
    }
    ...
    public void delete(Key key) {
        ...
        if(vals[i] != null) { // if the key has a value, change it to null
        	vals[i] = null;
            n--;
        } else { return; } // if the key is already a lazy deletion, return
    ...
    public Iterable<Key> keys() {
        ...
        for (int i = 0; i < m; i++)
            if (keys[i] != null && vals[i] != null) queue.add(keys[i]); // makes sure only valid keys are included
        return queue;
    }
    ...
    public boolean isLazyDeleted(int i) {
    	return keys[i] != null && vals[i] == null;
    }
